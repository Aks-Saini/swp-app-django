...
from .models import Approval, Package, Truck
...
class ApprovalAdmin(admin.ModelAdmin):
    ...
    def approve_selected(self, request, queryset):
        ...
        for approval in pending_approvals:
            try:
                ...
                response = requests.post(
                    "http://localhost:8000/api/visualize-packages/", json={"truck_id": str(approval.approval_truck.id)}
                )
                print(response.json())
                ...
                if response.ok:
                    ...
                    for box_id in allocated_boxes:
                        try:
                            ...
                            print(
                                f"Package with name {box_id} and destination {approval.approval_truck.destination} does not exist."
                            )
                else:
                    self.message_user(
                        request,
                        f"Failed to get allocated boxes for truck {approval.approval_truck.id}.",
                ...
                self.message_user(
                    request, f"Error occurred while communicating with the API: {str(e)}", level=messages.ERROR
                )
                continue
        ...
        if updated_count:
            ...
            self.message_user(
                request,
                "No pending approvals selected or all selected items are already processed.",
                level=messages.ERROR,
            )
